...
function toFlowElements(
  ...
  callbacks: { onDelete: (id: string) => void; onLabelChange: (id: string, l: string) => void; }
) {
  const flowNodes: Node[] = nodes.map(n => ({
    id: n.id, type: 'mf', position: { x: 0, y: 0 },
    data: { label: n.label, nodeType: n.nodeType, nodeId: n.id, description: n.description, onDelete: callbacks.onDelete, onLabelChange: callbacks.onLabelChange },
  }));
...
export default function MainframeFlowEditor({ processId, initialStepId, initialFlowId }: Props) {
  ...
  const callbacks = useMemo(() => ({ onDelete: handleDeleteNode, onLabelChange: handleLabelChange }), [handleDeleteNode, handleLabelChange]);

  const initial = useMemo(() => toFlowElements(workingNodes, workingConns, callbacks), []);
  ...
  const prevRef = useRef({ nodes: workingNodes, connections: workingConns });
  if (workingNodes !== prevRef.current.nodes || workingConns !== prevRef.current.connections) {
    prevRef.current = { nodes: workingNodes, connections: workingConns };
    const { nodes: ln, edges: le } = toFlowElements(workingNodes, workingConns, callbacks);
  ...
  const autoLayout = useCallback(() => {
    const { nodes: ln, edges: le } = toFlowElements(workingNodesRef.current, workingConnsRef.current, callbacks);
          ...
          <div className="flex-1 relative">
            ...
            >
              ...
              <Panel position="bottom-right" className="text-[10px] text-muted-foreground bg-background/80 px-2 py-1 rounded">
                Double-click label to edit · Drag handle to connect · Delete key to remove
              </Panel>
            </ReactFlow>
                  ...
                  <div className="p-1">
                    {history.map((entry, idx) => (
                      <button key={entry.id} onClick={() => goToHistory(idx)}
                        ...
                        <span className="text-[9px] text-muted-foreground ml-3.5">{entry.timestamp.toLocaleTimeString()}</span>
                      </button>
                    ))}
                  </div>
                </ScrollArea>
